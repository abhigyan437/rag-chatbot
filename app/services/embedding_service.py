...
Answer:"""
    ...
    def _generate_fallback_response(self, query: str, context_chunks: List[str], sources: List[DocumentSource]) -> str:
        ...
        source_refs = []
        for i, source in enumerate(sources):
            source_refs.append(f"[{i+1}] {source.filename} (chunk {source.chunk_id}, similarity: {source.similarity_score:.2f})")
        
        source_text = "\n".join(source_refs)
        
        response = f"""Based on the retrieved documents, here's the relevant information for your query: "{query}"
...
Note: This is a simple text extraction. For better responses, please configure an OpenAI API key."""
